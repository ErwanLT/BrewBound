...
import {BEER_STYLES} from "@/src/constants/beerStyles.ts";
...
export default function App() {
  ...
  const handleContribute = async (e?: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // For brewery additions in wizard mode, we might want to send the brewery AND the beers.
      // However, our backend currently handles one item at a time.
      // For simplicity in this first V2 step, we'll send the brewery first.
      // (The user could later add beers one by one, or we could update the backend to handle batch contributions).
      ...
      if (result.success) {
        // If it's a brewery addition with beers, we might need a more complex strategy,
        // but for now let's stick to the PR model.
            ...
            >
              <div className="p-6 md:p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 shrink-0">
                ...
                <button onClick={() => {
                  ...
                  setContributionSuccess(false);
                  setPrUrl(null);
                }} className="bg-white border border-slate-200 p-2 rounded-full hover:bg-slate-50 transition-all text-slate-400 hover:text-slate-900 shrink-0">
                  <X className="w-5 h-5" />
                </button>
              </div>
                        ...
                        {contributionStep === 4 && (
                          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                            ...
                            <div className="flex gap-4">
                              ...
                              <button onClick={() => handleContribute()} className="flex-[2] py-4 bg-emerald-500 text-white font-black uppercase tracking-widest rounded-2xl flex items-center justify-center gap-2 hover:bg-emerald-600 transition-all shadow-xl shadow-emerald-500/20 active:scale-95">
